...
if [[ ! -f "${PARQUET_LOCAL}" ]]; then
  ...
  echo "Usage: ./run-cutile-quote-spread.sh [local_parquet_path] [max_rows]"
  exit 1
fi
...
RUN_CMD="cargo run --release --bin cutile_quote_spread --features gpu-cutile -- pcap_samples/$(basename "${PARQUET_LOCAL}")"
if [[ -n "${MAX_ROWS}" ]]; then
  RUN_CMD="${RUN_CMD} ${MAX_ROWS}"
fi

echo "Running GPU quote spread aggregation on EC2..."
